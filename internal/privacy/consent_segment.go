...
func (e *Engine) ListConsentStates(ctx context.Context, f ConsentSegmentFilter, p httpapi.ListParams) (httpapi.Page[SubjectConsent], error) {
	p = p.Norm()
	...
	for round := 0; round < segmentScanRounds && len(matched) <= p.Limit; round++ {
		batch, err := e.scanConsentSegment(ctx, f, curUser, curPurpose, scanLimit)
		if err != nil {
			return zero, err
		}
		...
		if err := e.annotateReconfirm(ctx, batch); err != nil {
			return zero, err
		}
...
func (e *Engine) scanConsentSegment(ctx context.Context, f ConsentSegmentFilter, curUser, curPurpose string, limit int) ([]SubjectConsent, error) {
	const q = `
		with latest as (
		...
		limit $5`
	rows, err := e.pool.Query(ctx, q, f.Purpose, f.Granted, curUser, curPurpose, limit)
	if err != nil {
...
func (e *Engine) annotateReconfirm(ctx context.Context, batch []SubjectConsent) error {
	if len(batch) == 0 {
		return nil
	}
	...
	rows, err = e.pool.Query(ctx, `select user_id::text, purpose, max(created_at)
		from dilion_privacy.consent_events
		where action = $1 and user_id = any($2::uuid[])
		group by user_id, purpose`, ConsentReconfirmNotice, users)
	if err != nil {
...
func (e *Engine) ExportConsentAudience(ctx context.Context, purpose string, p httpapi.ListParams) (httpapi.Page[AudienceMember], error) {
	p = p.Norm()
	...
	const q = `
		with latest as (
			...
			from dilion_privacy.consent_events
			where purpose = $1 and action in ('GRANT','WITHDRAW')
		...
		where l.action = 'GRANT'
		  and ($2 = '' or l.user_id::text > $2)
		order by l.user_id::text
		limit $3`
	rows, err := e.pool.Query(ctx, q, purpose, cur, p.Limit+1)
	if err != nil {

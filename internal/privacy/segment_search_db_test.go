...
func TestConsentSegmentProjection(t *testing.T) {
	...
	page, err := env.e.ListConsentStates(env.ctx,
	...
	cursor := ""
	for i := 0; i < 10; i++ {
		p, err := env.e.ListConsentStates(env.ctx, ConsentSegmentFilter{},
...
func TestExportConsentAudience(t *testing.T) {
	...
	page, err := env.e.ExportConsentAudience(env.ctx, "marketing", httpapi.ListParams{})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := env.e.ExportConsentAudience(env.ctx, "  ", httpapi.ListParams{}); err == nil {
...
func TestSearchUsersByAuthIdentifier(t *testing.T) {
	...
	page, err := env.e.SearchUsers(env.ctx,
	...
	page, err = env.e.SearchUsers(env.ctx, UserSearchQuery{Email: email}, httpapi.ListParams{})
	if err != nil {
		t.Fatal(err)
	}
	if len(page.Items) != 0 {
	...
	if _, err := env.e.SearchUsers(env.ctx,
		UserSearchQuery{Email: email, Phone: "+821000000000"}, httpapi.ListParams{}); err == nil {
		t.Fatal("two criteria must be rejected")
	}
	if _, err := env.e.SearchUsers(env.ctx, UserSearchQuery{}, httpapi.ListParams{}); err == nil {
...
func TestSearchUsersByProfileFieldAndIndexLifecycle(t *testing.T) {
	...
	page, err := env.e.SearchUsers(env.ctx, q, httpapi.ListParams{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestListRequestsFilters(t *testing.T) {
	...
	byUser, err := env.e.ListRequests(env.ctx, RequestFilter{UserID: &a}, httpapi.ListParams{})
	if err != nil {
		t.Fatal(err)
	}
	...
	exp := RequestExport
	byType, err := env.e.ListRequests(env.ctx, RequestFilter{Type: &exp}, httpapi.ListParams{})
	if err != nil {
		t.Fatal(err)
	}
	...
	window, err := env.e.ListRequests(env.ctx,
...
func TestListHoldsActiveFilter(t *testing.T) {
	...
	if _, err := env.e.ReleaseHold(env.ctx, h1.ID, "admin"); err != nil {
		t.Fatal(err)
	}

	active := true
	page, err := env.e.ListHolds(env.ctx, HoldFilter{Active: &active}, httpapi.ListParams{})
	if err != nil {
		t.Fatal(err)
	}
	...
	released := false
	page, err = env.e.ListHolds(env.ctx, HoldFilter{Active: &released}, httpapi.ListParams{})
	if err != nil {
		t.Fatal(err)
	}
	...
	page, err = env.e.ListHolds(env.ctx, HoldFilter{}, httpapi.ListParams{})
	if err != nil {
		t.Fatal(err)
	}
	if len(page.Items) != 2 {

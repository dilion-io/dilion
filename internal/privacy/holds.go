...
func (e *Engine) CreateHold(ctx context.Context, in CreateHoldInput) (*LegalHold, error) {
	...
	const ins = `insert into dilion_privacy.legal_holds
		(id, user_id, domain, reason, basis, created_by, created_at)
		values ($1,$2::uuid,$3,$4,nullif($5,''),nullif($6,''),$7) returning ` + holdCols
	h, err := scanHold(e.pool.QueryRow(ctx, ins, httpapi.NewID("hold"), uid, in.Domain,
...
func (e *Engine) ReleaseHold(ctx context.Context, id, releasedBy string) (*LegalHold, error) {
	const upd = `update dilion_privacy.legal_holds
		set released_at = $2, released_by = nullif($3,'')
		where id = $1 and released_at is null
		returning ` + holdCols
	h, err := scanHold(e.pool.QueryRow(ctx, upd, id, e.now(), releasedBy))
	if errors.Is(err, pgx.ErrNoRows) {
		if _, gerr := e.getHold(ctx, id); gerr != nil {
			return nil, gerr
		}
...
func (e *Engine) getHold(ctx context.Context, id string) (*LegalHold, error) {
	const q = `select ` + holdCols + ` from dilion_privacy.legal_holds where id = $1`
	h, err := scanHold(e.pool.QueryRow(ctx, q, id))
	if errors.Is(err, pgx.ErrNoRows) {

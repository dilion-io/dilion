...
func TestCreateRequestGraceImmediateAndDuplicates(t *testing.T) {
	...
	if _, err := env.e.CancelRequest(env.ctx, "default", req.ID); !errors.Is(err, ErrConflict) {
		t.Errorf("second cancel err = %v, want ErrConflict", err)
	}
	if _, err := env.e.CancelRequest(env.ctx, "default", "pr_deadbeef"); !errors.Is(err, ErrNotFound) {
...
func TestLegalHoldGatesPipelineAndResumes(t *testing.T) {
	...
	if _, err := env.e.ReleaseHold(env.ctx, "default", hold.ID, "admin_2"); err != nil {
		t.Fatalf("release: %v", err)
	}
	if s, _ := env.status(t, req.ID); s != StatusRequested {
		t.Fatalf("status after release = %s, want REQUESTED (auto-resume)", s)
	}
	if _, err := env.e.ReleaseHold(env.ctx, "default", hold.ID, "admin_2"); !errors.Is(err, ErrConflict) {
...
func TestOutboxUserDeletedCreatesRequest(t *testing.T) {
	...
	insert := func() string {
		var id string
		payload := fmt.Sprintf(`{"user_id":%q,"project_id":"default","requested_by":"admin"}`, user)
...
func TestConsentLedger(t *testing.T) {
	...
	states, err := env.e.GetConsents(env.ctx, "default", user)
	if err != nil {
...
func TestReconfirmOutboxFansOutToWebhook(t *testing.T) {
	...
	payload := fmt.Sprintf(`{"user_id":%q,"project_id":"default","purpose":"marketing.email"}`, user)
...
func TestRetentionScannerShredsConsentKeys(t *testing.T) {
	...
	for _, u := range []string{due, held, future} {
		if _, err := env.e.UpdateConsent(env.ctx, "default", u, ConsentChange{
...
func TestDestinationsCRUDAndListing(t *testing.T) {
	...
	first, err := env.e.ListDestinations(env.ctx, "default", httpapi.ListParams{Limit: 2})
	if err != nil {
		t.Fatal(err)
	}
	...
	second, err := env.e.ListDestinations(env.ctx, "default", httpapi.ListParams{Limit: 2, Cursor: *first.NextCursor})
	if err != nil {
		t.Fatal(err)
	}
	...
	off := false
	upd, err := env.e.UpdateDestination(env.ctx, "default", created[0], &off,
	...
	if err := env.e.DeleteDestination(env.ctx, "default", created[0]); err != nil {
		t.Fatal(err)
	}
	if _, err := env.e.GetDestination(env.ctx, "default", created[0]); !errors.Is(err, ErrNotFound) {
		t.Errorf("get deleted err = %v", err)
	}
	if err := env.e.DeleteDestination(env.ctx, "default", created[0]); !errors.Is(err, ErrNotFound) {
		t.Errorf("delete twice err = %v", err)
	}
	// Cross-project reads look like "not found" (api-conventions).
	if _, err := env.e.GetDestination(env.ctx, "other", created[1]); !errors.Is(err, ErrNotFound) {
		t.Errorf("cross-project get err = %v", err)
	}
}
...
func TestListRequestsPagingAndFilter(t *testing.T) {
	...
	if _, err := env.e.CancelRequest(env.ctx, "default", ids[0]); err != nil {
		t.Fatal(err)
	}
	...
	got, err := env.e.GetRequest(env.ctx, "default", ids[1])
	if err != nil || got.ID != ids[1] {
		t.Fatalf("get = %+v (%v)", got, err)
	}
	if _, err := env.e.GetRequest(env.ctx, "other", ids[1]); !errors.Is(err, ErrNotFound) {
		t.Errorf("cross-project get err = %v", err)
	}
}

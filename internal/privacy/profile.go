...
func (e *Engine) GetProfile(ctx context.Context, userID string, full bool) (*Profile, error) {
...
func (e *Engine) UpdateProfile(ctx context.Context, userID string, set map[string]ProfileField, remove []string) (*Profile, error) {
	uid, err := validUUID(userID)
	if err != nil {
		return nil, err
	}
	...
	for attempt := 0; attempt < 64; attempt++ {
		p, retry, err := e.patchProfile(ctx, uid, set, remove)
		if !retry {
			return p, err
		}
...
func (e *Engine) patchProfile(ctx context.Context, uid string, set map[string]ProfileField, remove []string) (*Profile, bool, error) {
	...
	e.log.Info("pii profile updated", "user_id", uid, "fields", len(fields))
	if len(fields) == 0 {

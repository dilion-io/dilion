...
func (e *Engine) processRequest(ctx context.Context, requestID string) error {
	const q = `select id, project_id, user_id::text, status, policy_snapshot, scheduled_at
	...
	err := e.pool.QueryRow(ctx, q, requestID).
		Scan(&rc.RequestID, &rc.ProjectID, &rc.UserID, &status, &snapshot, &scheduledAt)
	...
	if held {
		...
		_ = e.runHook(ctx, ports.BeforeErasureStep, map[string]any{
			...
			"user_id": rc.UserID, "project_id": rc.ProjectID,
		})
		return nil
	}
	...
	for _, st := range e.steps() {
		...
		if err := e.runHook(ctx, ports.BeforeErasureStep, map[string]any{
			...
			"project_id": rc.ProjectID, "domain": st.Domain, "action": string(action),
			"step": st.Order,
		}); err != nil {
	...
	if err := e.runHook(ctx, ports.AfterErasure, map[string]any{
		"request_id": rc.RequestID, "user_id": rc.UserID, "project_id": rc.ProjectID,
		"completed_at": e.now(),
	}); err != nil {
...
func (e *Engine) stepExternalSystem(ctx context.Context, rc *runCtx, _ ErasureAction) (string, bool, error) {
	rows, err := e.pool.Query(ctx,
		`select id from dilion_privacy.destinations where project_id = $1 and enabled order by id`,
		rc.ProjectID)
	if err != nil {

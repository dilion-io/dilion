...
func (e *Engine) CreateDestination(ctx context.Context, in CreateDestinationInput) (*Destination, error) {
	...
	const ins = `insert into dilion_privacy.destinations
		(id, project_id, type, name, config, secret_enc, enabled, created_at)
		values ($1,$2,$3,$4,$5::jsonb,$6,true,$7) returning ` + destinationCols
	d, err := scanDestination(e.pool.QueryRow(ctx, ins, httpapi.NewID("dst"), pid, in.Type, in.Name,
...
func (e *Engine) GetDestination(ctx context.Context, projectID, id string) (*Destination, error) {
	const q = `select ` + destinationCols + ` from dilion_privacy.destinations
		where project_id = $1 and id = $2`
	d, err := scanDestination(e.pool.QueryRow(ctx, q, normProject(projectID), id))
...
func (e *Engine) ListDestinations(ctx context.Context, projectID string, p httpapi.ListParams) (httpapi.Page[Destination], error) {
	p = p.Norm()
	...
	const q = `select ` + destinationCols + ` from dilion_privacy.destinations
		...
		order by id limit $3`
	rows, err := e.pool.Query(ctx, q, normProject(projectID), cur, p.Limit+1)
	if err != nil {
...
func (e *Engine) UpdateDestination(ctx context.Context, projectID, id string, enabled *bool, config map[string]any) (*Destination, error) {
	cur, err := e.GetDestination(ctx, projectID, id)
	if err != nil {
		return nil, err
	}
	next := cur.Config
	...
	const upd = `update dilion_privacy.destinations set config = $3::jsonb, enabled = $4
		where project_id = $1 and id = $2 returning ` + destinationCols
	d, err := scanDestination(e.pool.QueryRow(ctx, upd, normProject(projectID), id, string(raw), en))
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, ErrNotFound
	}
	if err != nil {
...
func (e *Engine) DeleteDestination(ctx context.Context, projectID, id string) error {
	tag, err := e.pool.Exec(ctx, `delete from dilion_privacy.destinations where project_id = $1 and id = $2`,
		normProject(projectID), id)
	if err != nil {

...
type Service interface {
	...
	GetDestination(ctx context.Context, projectID, id string) (*Destination, error)
	ListDestinations(ctx context.Context, projectID string, p httpapi.ListParams) (httpapi.Page[Destination], error)
	UpdateDestination(ctx context.Context, projectID, id string, enabled *bool, config map[string]any) (*Destination, error)
	DeleteDestination(ctx context.Context, projectID, id string) error

	// PII Profile (custom field 지원; full=false는 hint 기반 masked 프로젝션.
	// full=true는 호출자가 pii.reveal 권한·사유·PII_FULL_READ 감사를 책임진다.)
	GetProfile(ctx context.Context, projectID, userID string, full bool) (*Profile, error)
	// UpdateProfile: set은 upsert, remove는 키 삭제. masked 프로젝션을 반환.
	UpdateProfile(ctx context.Context, projectID, userID string, set map[string]ProfileField, remove []string) (*Profile, error)

...
func (e *Engine) CreateRequest(ctx context.Context, in CreateRequestInput) (*Request, error) {
	...
	if in.IdempotencyKey != "" {
		prev, err := e.requestByIdempotencyKey(ctx, in.IdempotencyKey)
		if err != nil {
			return nil, err
		}
		if prev != nil {
	...
	const ins = `insert into dilion_privacy.personal_data_requests
		(id, user_id, type, status, policy_id, policy_snapshot, scheduled_at,
...
func (e *Engine) requestByIdempotencyKey(ctx context.Context, key string) (*Request, error) {
	const q = `select ` + requestCols + `
		from dilion_privacy.personal_data_requests
		where idempotency_key = $1`
	r, err := scanRequest(e.pool.QueryRow(ctx, q, key))
...
// GetRequest returns one request; a missing id is "not found" per
// api-conventions.
func (e *Engine) GetRequest(ctx context.Context, id string) (*Request, error) {
	const q = `select ` + requestCols + ` from dilion_privacy.personal_data_requests
		where id = $1`
	r, err := scanRequest(e.pool.QueryRow(ctx, q, id))
	if errors.Is(err, pgx.ErrNoRows) {
...
func (e *Engine) CancelRequest(ctx context.Context, id string) (*Request, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		...
		if _, gerr := e.GetRequest(ctx, id); gerr != nil {
			return nil, gerr
		}
		return nil, ErrConflict
	}

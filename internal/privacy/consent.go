...
func (e *Engine) GetConsents(ctx context.Context, projectID, userID string) ([]ConsentState, error) {
	...
	const q = `select distinct on (purpose)
		...
		where project_id = $1 and user_id = $2::uuid and action in ('GRANT','WITHDRAW')
		order by purpose, created_at desc, id desc`
	rows, err := e.pool.Query(ctx, q, pid, uid)
	if err != nil {
...
func (e *Engine) lastReconfirmNotices(ctx context.Context, projectID, userID string) (map[string]*time.Time, error) {
	const q = `select purpose, max(created_at) from dilion_privacy.consent_events
		where project_id = $1 and user_id = $2::uuid and action = $3 group by purpose`
	rows, err := e.pool.Query(ctx, q, projectID, userID, ConsentReconfirmNotice)
	if err != nil {
...
func (e *Engine) UpdateConsent(ctx context.Context, projectID, userID string, ch ConsentChange) (*ConsentState, error) {
	pid := normProject(projectID)
	uid, err := validUUID(userID)
	if err != nil {
		return nil, err
...
func (e *Engine) appendConsentEvent(ctx context.Context, q queryExecer, ev consentEvent) error {
	const ins = `insert into dilion_privacy.consent_events
		(project_id, user_id, purpose, action, policy_version, created_at, source, region, evidence)
		values ($1,$2::uuid,$3,$4,nullif($5,''),$6,nullif($7,''),nullif($8,''),$9::jsonb)`
	_, err := q.Exec(ctx, ins, ev.ProjectID, ev.UserID, ev.Purpose, ev.Action, ev.PolicyVersion,

...
func TestAdminGateAcceptsUserTokenWithUsersAdmin(t *testing.T) {
	...
	got := authz.seen[0]
	if got.ID != sub || got.Type != ActorTypeUser || got.ProjectID != DefaultProjectID {
		t.Errorf("actor = %+v, want {ID:%s Type:user ProjectID:default}", got, sub)
	}
}
...
func TestAdminGateWithRealAuthorizerAndRoleAssignment(t *testing.T) {
	...
	if err := env.pool.QueryRow(ctx, `
		insert into dilion_authz.role_assignments (project_id, actor_id, role_id, granted_by)
		values ('default', $1, $2, 'test') returning id`,

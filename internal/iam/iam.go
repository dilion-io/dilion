...
// DefaultProjectID — wave 1 is single-project (PLAN.md §3.5).
...
func (s *Service) CreatePermission(ctx context.Context, projectID, name string) (Permission, error) {
	pool, err := s.db(ctx)
	if err != nil {
		return Permission{}, err
	}
	projectID = projectOr(projectID)
	if err := ValidateCustomPermission(name); err != nil {
		return Permission{}, err
	}
	p := Permission{ProjectID: projectID, Name: name}
	err = pool.QueryRow(ctx, `
		insert into dilion_authz.permissions (project_id, name, builtin)
		values ($1,$2,false)
		on conflict (project_id, name) do nothing
		returning created_at`, projectID, name).Scan(&p.CreatedAt)
...
func (s *Service) ListPermissions(ctx context.Context, projectID string, p httpapi.ListParams) (httpapi.Page[Permission], error) {
	...
	rows, err := pool.Query(ctx, `
		select project_id, name, builtin, created_at
		from dilion_authz.permissions
		where project_id = $1 and ($2 = '' or name > $2)
		order by name asc limit $3`, projectOr(projectID), after, p.Limit+1)
	if err != nil {
	...
	for rows.Next() {
		var it Permission
		if err := rows.Scan(&it.ProjectID, &it.Name, &it.Builtin, &it.CreatedAt); err != nil {
			return page, fmt.Errorf("iam: list permissions: %w", err)
		}
...
// registered (builtin seed or a custom permission) in the project.
func (s *Service) CreateRole(ctx context.Context, projectID, name string, permissions []string) (Role, error) {
	pool, err := s.db(ctx)
	if err != nil {
		return Role{}, err
	}
	...
	if err := pool.QueryRow(ctx, `
		...
		from dilion_authz.permissions
		where project_id = $1 and name = any($2)`, projectID, perms).Scan(&known); err != nil {
	...
	r := Role{ID: httpapi.NewID("role"), ProjectID: projectID, Name: name, Permissions: perms}
	err = pool.QueryRow(ctx, `
		insert into dilion_authz.roles (id, project_id, name, permissions, builtin)
		values ($1,$2,$3,$4,false)
		on conflict (project_id, name) do nothing
		returning created_at`, r.ID, projectID, name, perms).Scan(&r.CreatedAt)
	if errors.Is(err, pgx.ErrNoRows) {
...
func (s *Service) GetRole(ctx context.Context, projectID, roleID string) (Role, error) {
	pool, err := s.db(ctx)
	if err != nil {
		return Role{}, err
	}
	var r Role
	err = pool.QueryRow(ctx, `
		select id, project_id, name, permissions, builtin, created_at
		from dilion_authz.roles where project_id = $1 and id = $2`,
		projectOr(projectID), roleID).
		Scan(&r.ID, &r.ProjectID, &r.Name, &r.Permissions, &r.Builtin, &r.CreatedAt)
	if errors.Is(err, pgx.ErrNoRows) {
...
func (s *Service) ListRoles(ctx context.Context, projectID string, p httpapi.ListParams) (httpapi.Page[Role], error) {
	...
	for rows.Next() {
		var it Role
		if err := rows.Scan(&it.ID, &it.ProjectID, &it.Name, &it.Permissions, &it.Builtin, &it.CreatedAt); err != nil {
			return page, fmt.Errorf("iam: list roles: %w", err)
		}
		items = append(items, it)
	}
...
func (s *Service) GrantRole(ctx context.Context, projectID, roleID, actorID, grantedBy string) (Assignment, error) {
	pool, err := s.db(ctx)
	if err != nil {
	...
	if _, err := s.GetRole(ctx, projectID, roleID); err != nil {
		return Assignment{}, err
	}
	var active bool
	if err := pool.QueryRow(ctx, `
		select exists(select 1 from dilion_authz.role_assignments
		              where project_id = $1 and role_id = $2 and actor_id = $3 and revoked_at is null)`,
		projectID, roleID, actorID).Scan(&active); err != nil {
	...
	a := Assignment{ProjectID: projectID, ActorID: actorID, RoleID: roleID}
	if grantedBy != "" {
		a.GrantedBy = &grantedBy
	}
	if err := pool.QueryRow(ctx, `
		insert into dilion_authz.role_assignments (project_id, actor_id, role_id, granted_by, granted_at)
		values ($1,$2,$3,$4,$5) returning id, granted_at`,
		projectID, actorID, roleID, a.GrantedBy, s.clock()).Scan(&a.ID, &a.GrantedAt); err != nil {
		return Assignment{}, fmt.Errorf("iam: grant role: %w", err)
	}
	return a, nil
}
...
func (s *Service) RevokeAssignment(ctx context.Context, projectID string, id int64, revokedBy string) (Assignment, error) {
	pool, err := s.db(ctx)
	if err != nil {
		return Assignment{}, err
	}
	...
	err = pool.QueryRow(ctx, `
		...
		returning id, project_id, actor_id, role_id, granted_by, granted_at, revoked_by, revoked_at`,
		projectID, id, by, s.clock()).
		Scan(&a.ID, &a.ProjectID, &a.ActorID, &a.RoleID, &a.GrantedBy, &a.GrantedAt, &a.RevokedBy, &a.RevokedAt)
	if errors.Is(err, pgx.ErrNoRows) {
		...
		if err2 := pool.QueryRow(ctx, `select exists(select 1 from dilion_authz.role_assignments
			...
			projectID, id).Scan(&exists); err2 != nil {
...
// AssignmentFilter narrows ListAssignments. Zero value = active assignments of
// the whole project.
...
func (s *Service) ListAssignments(ctx context.Context, projectID string, f AssignmentFilter, p httpapi.ListParams) (httpapi.Page[Assignment], error) {
	...
	rows, err := pool.Query(ctx, `
		select id, project_id, actor_id, role_id, granted_by, granted_at, revoked_by, revoked_at
		...
		projectOr(projectID), f.RoleID, f.ActorID, f.IncludeRevoked, afterID, p.Limit+1)
	if err != nil {
	...
	for rows.Next() {
		var it Assignment
		if err := rows.Scan(&it.ID, &it.ProjectID, &it.ActorID, &it.RoleID,
...
func (s *Service) Recertification(ctx context.Context, projectID, permission string) ([]Holder, error) {
	pool, err := s.db(ctx)
	if err != nil {
		return nil, err
	}
	...
	rows, err := pool.Query(ctx, `
		...
		where ra.project_id = $1 and ra.revoked_at is null and $2 = any(r.permissions)
		order by ra.actor_id asc, ra.granted_at asc`, projectOr(projectID), permission)
	if err != nil {

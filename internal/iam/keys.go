...
func (s *Service) CreateKey(ctx context.Context, projectID, name string, scopes []string, expires *time.Time) (string, APIKey, error) {
	return s.createKey(ctx, projectID, name, scopes, expires, "")
}

// CreateKeyBy is CreateKey with the granting actor recorded.
func (s *Service) CreateKeyBy(ctx context.Context, projectID, name string, scopes []string, expires *time.Time, createdBy string) (string, APIKey, error) {
	return s.createKey(ctx, projectID, name, scopes, expires, createdBy)
}

func (s *Service) createKey(ctx context.Context, projectID, name string, scopes []string, expires *time.Time, createdBy string) (string, APIKey, error) {
	pool, err := s.db(ctx)
	if err != nil {
		return "", APIKey{}, err
	}
	...
	// permission the project does not know about.
	var known []string
	if err := pool.QueryRow(ctx, `
		select coalesce(array_agg(name), '{}')
		from dilion_authz.permissions
		where project_id = $1 and name = any($2)`, projectID, sc).Scan(&known); err != nil {
	...
	if err := pool.QueryRow(ctx, `
		insert into dilion_authz.api_keys
			(id, project_id, name, key_hash, scopes, created_at, created_by, expires_at)
		values ($1,$2,$3,$4,$5,$6,$7,$8)
		returning created_at`,
		k.ID, projectID, k.Name, HashToken(token), sc, now, k.CreatedBy, expires).Scan(&k.CreatedAt); err != nil {
		return "", APIKey{}, fmt.Errorf("iam: create key: %w", err)
	}
	return token, k, nil
}
...
func (s *Service) VerifyKey(ctx context.Context, token string) (*ports.Actor, []string, error) {
	...
	err = pool.QueryRow(ctx, `
		select id, project_id, scopes, key_hash, expires_at, revoked_at
		from dilion_authz.api_keys where key_hash = $1`, hash).
		Scan(&id, &projectID, &scopes, &storedHsh, &expiresAt, &revokedAt)
	...
	return &ports.Actor{ID: id, Type: ActorTypeAPIKey, ProjectID: projectID}, scopes, nil
}
...
func (s *Service) RevokeKey(ctx context.Context, projectID, keyID, revokedBy string) (APIKey, error) {
	pool, err := s.db(ctx)
	if err != nil {
		return APIKey{}, err
	}
	...
	var k APIKey
	err = pool.QueryRow(ctx, `
		update dilion_authz.api_keys set revoked_at = $3, revoked_by = $4
		where project_id = $1 and id = $2 and revoked_at is null
		returning id, project_id, name, scopes, created_at, created_by, expires_at, revoked_at, last_used_at`,
		projectID, keyID, s.clock(), by).
		Scan(&k.ID, &k.ProjectID, &k.Name, &k.Scopes, &k.CreatedAt, &k.CreatedBy, &k.ExpiresAt, &k.RevokedAt, &k.LastUsedAt)
	if errors.Is(err, pgx.ErrNoRows) {
		var exists bool
		if err2 := pool.QueryRow(ctx, `select exists(select 1 from dilion_authz.api_keys
		                                             where project_id = $1 and id = $2)`,
			projectID, keyID).Scan(&exists); err2 != nil {
...
func (s *Service) ListKeys(ctx context.Context, projectID string, p httpapi.ListParams) (httpapi.Page[APIKey], error) {
	...
	rows, err := pool.Query(ctx, `
		select id, project_id, name, scopes, created_at, created_by, expires_at, revoked_at, last_used_at
		from dilion_authz.api_keys
		where project_id = $1 and ($2 = '' or id > $2)
		order by id asc limit $3`, projectOr(projectID), after, p.Limit+1)
	if err != nil {
	...
	for rows.Next() {
		var k APIKey
		if err := rows.Scan(&k.ID, &k.ProjectID, &k.Name, &k.Scopes, &k.CreatedAt,

...
func TestSeededBuiltins(t *testing.T) {
	...
	perms, err := svc.ListPermissions(ctx, httpapi.ListParams{Limit: 100})
	if err != nil {
	...
	roles, err := svc.ListRoles(ctx, httpapi.ListParams{Limit: 100})
	if err != nil {
...
func TestAuthorizerGrantsUserActorByAssignment(t *testing.T) {
	...
	if _, err := svc.RevokeAssignment(ctx, a.ID, "admin_root"); err != nil {
...
func TestAuthorizerUsesActiveAssignments(t *testing.T) {
	...
	actor := ports.Actor{ID: "admin_alice", Type: ActorTypeAdmin}
	...
	a, err := svc.GrantRole(ctx, RoleSupport, actor.ID, "admin_root")
	if err != nil {
	...
	active, err := svc.ListAssignments(ctx, AssignmentFilter{ActorID: actor.ID}, httpapi.ListParams{})
	if err != nil {
	...
	history, err := svc.ListAssignments(ctx,
...
func TestGrantRoleErrors(t *testing.T) {
	...
	if _, err := svc.GrantRole(ctx, "role_ffffffffffffffffffffffffffffffff", "a", "root"); !errors.Is(err, ErrNotFound) {
		t.Errorf("unknown role: err = %v, want ErrNotFound", err)
	}
	if _, err := svc.GrantRole(ctx, RoleViewer, "  ", "root"); !errors.Is(err, ErrInvalid) {
		t.Errorf("blank actor: err = %v, want ErrInvalid", err)
	}
	if _, err := svc.GrantRole(ctx, RoleViewer, "dup", "root"); err != nil {
		t.Fatalf("grant: %v", err)
	}
	if _, err := svc.GrantRole(ctx, RoleViewer, "dup", "root"); !errors.Is(err, ErrConflict) {
...
func TestRevokeAssignmentErrors(t *testing.T) {
	...
	if _, err := svc.RevokeAssignment(ctx, 999999, "root"); !errors.Is(err, ErrNotFound) {
		t.Errorf("unknown assignment: err = %v, want ErrNotFound", err)
	}
	a, err := svc.GrantRole(ctx, RoleViewer, "bob", "root")
	if err != nil {
		t.Fatalf("grant: %v", err)
	}
	...
	if _, err := svc.RevokeAssignment(ctx, a.ID, "root"); !errors.Is(err, ErrConflict) {
...
func TestCustomPermissionsAndRoles(t *testing.T) {
	...
	if _, err := svc.CreatePermission(ctx, "orders"); !errors.Is(err, ErrInvalid) {
		t.Errorf("un-namespaced permission: err = %v, want ErrInvalid", err)
	}
	if _, err := svc.CreatePermission(ctx, PermUsersRead); !errors.Is(err, ErrInvalid) {
		t.Errorf("builtin collision: err = %v, want ErrInvalid", err)
	}
	p, err := svc.CreatePermission(ctx, "myapp.orders.refund")
	if err != nil {
	...
	if _, err := svc.CreatePermission(ctx, "myapp.orders.refund"); !errors.Is(err, ErrConflict) {
		t.Errorf("duplicate permission: err = %v, want ErrConflict", err)
	}

	if _, err := svc.CreateRole(ctx, "refunder", []string{"myapp.nope"}); !errors.Is(err, ErrInvalid) {
		t.Errorf("unknown permission in role: err = %v, want ErrInvalid", err)
	}
	if _, err := svc.CreateRole(ctx, "refunder", nil); !errors.Is(err, ErrInvalid) {
		t.Errorf("empty role: err = %v, want ErrInvalid", err)
	}
	role, err := svc.CreateRole(ctx, "refunder",
	...
	if _, err := svc.CreateRole(ctx, "refunder", []string{PermUsersRead}); !errors.Is(err, ErrConflict) {
		t.Errorf("duplicate role name: err = %v, want ErrConflict", err)
	}

	// The custom permission is decidable by the Authorizer.
	authz := NewAuthorizer(pool)
	actor := ports.Actor{ID: "admin_carol", Type: ActorTypeAdmin}
	if _, err := svc.GrantRole(ctx, role.ID, actor.ID, "root"); err != nil {
		t.Fatalf("grant: %v", err)
	}
...
func TestRecertification(t *testing.T) {
	...
	if _, err := svc.GrantRole(ctx, RoleOwner, "admin_dana", "root"); err != nil {
		t.Fatalf("grant owner: %v", err)
	}
	if _, err := svc.GrantRole(ctx, RoleViewer, "admin_erin", "root"); err != nil {
		t.Fatalf("grant viewer: %v", err)
	}

	holders, err := svc.Recertification(ctx, PermPIIReveal)
	if err != nil {
	...
	readers, err := svc.Recertification(ctx, PermUsersRead)
	if err != nil {
		t.Fatalf("recertification: %v", err)
	}
	if len(readers) != 2 {
	...
	if _, err := svc.Recertification(ctx, " "); !errors.Is(err, ErrInvalid) {
...
func TestAPIKeyLifecycle(t *testing.T) {
	...
	token, key, err := svc.CreateKey(ctx, "ci", []string{PermUsersRead, PermPIIRead}, nil)
	if err != nil {
	...
	if actor.ID != key.ID || actor.Type != ActorTypeAPIKey {
	...
	if _, err := svc.RevokeKey(ctx, key.ID, "root"); !errors.Is(err, ErrConflict) {
		t.Errorf("double revoke: err = %v, want ErrConflict", err)
	}
	if _, err := svc.RevokeKey(ctx, "key_ffffffffffffffffffffffffffffffff", "root"); !errors.Is(err, ErrNotFound) {
...
func TestAPIKeyExpiry(t *testing.T) {
	...
	if _, _, err := svc.CreateKey(ctx, "stale", []string{PermUsersRead}, &past); !errors.Is(err, ErrInvalid) {
		t.Errorf("past expiry: err = %v, want ErrInvalid", err)
	}
	if _, _, err := svc.CreateKey(ctx, "noscope", nil, nil); !errors.Is(err, ErrInvalid) {
		t.Errorf("no scopes: err = %v, want ErrInvalid", err)
	}
	if _, _, err := svc.CreateKey(ctx, "bogus", []string{"myapp.unknown"}, nil); !errors.Is(err, ErrInvalid) {
		t.Errorf("unregistered scope: err = %v, want ErrInvalid", err)
	}

	soon := time.Now().UTC().Add(time.Hour)
	token, _, err := svc.CreateKey(ctx, "short", []string{PermUsersRead}, &soon)
	if err != nil {
		t.Fatalf("create key: %v", err)
	}
...
func TestListPaginationAcrossPages(t *testing.T) {
	...
	for _, actor := range []string{"a1", "a2", "a3"} {
		if _, err := svc.GrantRole(ctx, RoleViewer, actor, "root"); err != nil {
			t.Fatalf("grant: %v", err)
		}
	}
	first, err := svc.ListAssignments(ctx, AssignmentFilter{}, httpapi.ListParams{Limit: 2})
	if err != nil {
		t.Fatalf("list: %v", err)
	}
	...
	second, err := svc.ListAssignments(ctx, AssignmentFilter{},

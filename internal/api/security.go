	...
	default:
		...
		case iam.ActorTypeServiceRole:
			ri.Actor = ports.Actor{
				...
				ProjectID: iam.DefaultProjectID,
			}
		case iam.RoleAuthenticated:
			...
			ri.Actor = ports.Actor{ID: claims.Subject, Type: iam.ActorTypeUser, ProjectID: iam.DefaultProjectID}
		default:
			// Any other compatibility-plane JWT (anon, ...) carries no management
			// plane authority at all.
			ri.Actor = ports.Actor{ID: claims.Subject, Type: claims.Role, ProjectID: iam.DefaultProjectID}

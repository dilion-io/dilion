...
func TestAppendDefaultsProjectAndTimestamp(t *testing.T) {
	...
	if err := pool.QueryRow(ctx, `select project_id, created_at, actor_id from dilion_audit.events`).
		Scan(&projectID, &createdAt, &actorID); err != nil {
		t.Fatalf("read: %v", err)
	}
	if projectID != DefaultProjectID {
		t.Errorf("project_id = %q, want %q", projectID, DefaultProjectID)
	}
	if createdAt.IsZero() {

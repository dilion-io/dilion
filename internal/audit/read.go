...
// lists every event newest-first.
...
const eventColumns = `e.event_id, e.actor_id, e.actor_type, e.action,
	e.resource, e.access_level, e.request_id, e.result_count, e.reason,
	e.ip, e.user_agent, e.created_at`

func scanEvent(row pgx.Row) (Event, error) {
	var e Event
	err := row.Scan(&e.ID, &e.ActorID, &e.ActorType, &e.Action,
...
func (r *Reader) ListEvents(ctx context.Context, f EventFilter, p httpapi.ListParams) (httpapi.Page[Event], error) {
	...
	rows, err := pool.Query(ctx, `
		...
		where ($1::text is null or e.actor_id = $1)
...
// GetEvent returns one event together with its full subject manifest.
func (r *Reader) GetEvent(ctx context.Context, eventID string) (*Event, error) {
	pool, err := r.db(ctx)
	if err != nil {
		return nil, err
	}

...
func TestListEventsNewestFirstAndFiltered(t *testing.T) {
	...
	all, err := r.ListEvents(ctx, EventFilter{}, httpapi.ListParams{})
	if err != nil {
		t.Fatalf("list: %v", err)
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			page, err := r.ListEvents(ctx, tc.f, httpapi.ListParams{})
			if err != nil {
				t.Fatalf("list: %v", err)
			}
...
func TestListEventsBySubjectReverseLookup(t *testing.T) {
	...
	page, err := r.ListEvents(ctx, EventFilter{SubjectID: ptr(subjectA)}, httpapi.ListParams{})
	if err != nil {
		t.Fatalf("list: %v", err)
	}
	got := actionsOf(page.Items)
	...
	page, err = r.ListEvents(ctx, EventFilter{SubjectID: ptr(subjectB)}, httpapi.ListParams{})
	if err != nil {
		t.Fatalf("list: %v", err)
	}
	...
	page, err = r.ListEvents(ctx, EventFilter{SubjectID: ptr("no-such-subject")}, httpapi.ListParams{})
	if err != nil {
		t.Fatalf("list: %v", err)
	}
...
func TestListEventsPaginatesWithoutOverlap(t *testing.T) {
	...
	first, err := r.ListEvents(ctx, EventFilter{}, httpapi.ListParams{Limit: 2})
	if err != nil {
		t.Fatalf("list: %v", err)
	}
	...
	second, err := r.ListEvents(ctx, EventFilter{},
	...
	if _, err := r.ListEvents(ctx, EventFilter{},
...
func TestGetEventReturnsFullSubjectManifest(t *testing.T) {
	...
	page, err := r.ListEvents(ctx, EventFilter{Action: ptr(ActionUserListRead)}, httpapi.ListParams{})
	if err != nil || len(page.Items) != 1 {
		t.Fatalf("list: %v (%d items)", err, len(page.Items))
	}
	ev, err := r.GetEvent(ctx, page.Items[0].ID)
	if err != nil {
		t.Fatalf("get: %v", err)
	}
	...
	if _, err := r.GetEvent(ctx, httpapi.NewID("evt")); !errors.Is(err, ErrNotFound) {
...
func TestReasonIsPersistedAndReturned(t *testing.T) {
	...
	page, err := r.ListEvents(ctx, EventFilter{Action: ptr(ActionPIIFullRead)}, httpapi.ListParams{})
	if err != nil || len(page.Items) != 1 {
		t.Fatalf("list: %v (%d items)", err, len(page.Items))
	}
	got := page.Items[0]
...
func TestSystemEventHasNullActor(t *testing.T) {
	...
	page, err := NewReader(pool).ListEvents(ctx, EventFilter{}, httpapi.ListParams{})
	if err != nil || len(page.Items) != 1 {
		t.Fatalf("list: %v (%d items)", err, len(page.Items))
	}
	e := page.Items[0]
...
func TestReaderRequiresPool(t *testing.T) {
	r := NewReader(nil)
	if _, err := r.ListEvents(context.Background(), EventFilter{}, httpapi.ListParams{}); err == nil {
		t.Error("expected an error with a nil pool")
	}
	if _, err := r.GetEvent(context.Background(), "evt_x"); err == nil {
		t.Error("expected an error with a nil pool")
	}
}

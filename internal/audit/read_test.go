...
func TestListEventsTimeWindow(t *testing.T) {
	...
	page, err := r.ListEvents(ctx, DefaultProjectID, EventFilter{From: &from, To: &to}, httpapi.ListParams{})
	if err != nil {
		t.Fatalf("list: %v", err)
	}
	...
	page, err = r.ListEvents(ctx, DefaultProjectID, EventFilter{From: &exact, To: &exact}, httpapi.ListParams{})
	if err != nil {
		t.Fatalf("list: %v", err)
	}
	if len(page.Items) != 0 {

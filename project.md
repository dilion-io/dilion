...
- **코드로 정의하는 ID 연동 (커스터마이징 전용):** 테넌트별 IdP 등록과 OAuth 클라이언트를 이미 자기 데이터로 알고 있는 플랫폼이 이를 인스턴스 DB마다 복제하지 않도록, 두 방향 모두 코드 소스를 **DB보다 먼저** 조회합니다. 소스가 `(nil, nil)`을 돌려주면 기존 경로로 넘어가고, 오류는 요청을 실패시킵니다(넘어가지 않음).
  ...
  - **인스턴스 사이트 — `WithAuthSettingsSource`:** 인스턴스별 site URL과 redirect 허용 목록(`URIAllowList`)을 돌려줍니다. 메일 링크, redirect 판정, OAuth 동의 페이지 주소, SAML SP 주소, passkey origin 기본값, TOTP issuer가 모두 요청의 인스턴스 값을 따르며, `(nil, nil)`이면 서버 설정(`DILION_AUTH_SITE_URL`, `URI_ALLOW_LIST`)을 씁니다. 요청마다 한 번 조회하고, 컴파일한 허용 목록은 값이 바뀔 때까지 인스턴스별로 캐시합니다.
...
**MFA 계정의 단계 인증(step-up):** 검증된 MFA factor가 있는 사용자는 다음 작업에 aal2 세션이 필요합니다. 이메일·전화번호·비밀번호 변경(`PUT /user`, upstream과 같은 401 `insufficient_aal`), 로그인 수단(identity) 연결·해제(401 `insufficient_aal`), OAuth 2.1 동의(조회·자동 승인·승인 모두, 403 — upstream에는 없는 강화), passkey·OPAQUE 자격증명 관리, 본인 탈퇴. factor가 없는 사용자는 aal2에 도달할 수 없으므로 요구하지 않습니다. 익명 사용자는 upstream처럼 factor를 등록할 수 없고, 밴된 계정은 어떤 자격증명으로도 세션을 받지 못합니다. 이메일 자동 확인(`MAILER_AUTOCONFIRM`)은 upstream과 같이 기본 꺼짐입니다. 켜 두면 가입과 `PUT /user`가 확인되지 않은 주소를 곧바로 계정의 복구 주소로 삼습니다. 파기된(또는 soft delete된) 계정은 남은 자격증명이 무엇이든 세션을 발급받지 못하며, 같은 소셜 계정으로 다시 가입하면 새 계정이 만들어집니다.
...
- **부여 상한(grant ceiling):** 누구도 자기가 가지지 않은 permission을 줄 수 없습니다. API key의 scope, role 정의(생성·수정 시 기존과 새 권한 모두), role 할당·회수가 모두 호출자가 가진 permission의 부분집합이어야 하며, 어긋나면 403과 함께 부족한 permission을 알려 줍니다. 그래서 `roles.manage`나 `keys.manage`를 가져도 그보다 강한 권한(예: `owner`)으로 올라갈 수 없고, 자기보다 강한 사람의 role을 회수할 수도 없습니다. `service_role`은 전권이므로 예외입니다. 사용자가 발급한 API key는 발급자가 **지금** 가진 권한을 넘지 못하므로, 발급자의 role이 회수되거나 계정이 삭제되면 key도 그만큼 약해집니다. API key로 API key를 발급할 수는 없고(스스로 영원히 갱신할 수 있으므로), key 회수에도 부여 상한이 적용됩니다. 계정을 삭제하거나 파기하면 그 계정의 role 할당도 회수됩니다(이력은 남음).
...
- **신뢰할 proxy와 외부 호출 허용 망:** `X-Forwarded-For`는 클라이언트가 마음대로 보낼 수 있으므로, `DILION_TRUSTED_PROXIES`(`WithTrustedProxies`)에 적은 proxy가 전달한 경우에만 믿습니다. 값은 쉼표로 구분한 IP·CIDR이고 `*`는 모두 신뢰(기존 동작)입니다. 오른쪽부터 신뢰하는 proxy를 건너뛰고 처음 만나는 신뢰하지 않는 주소를 클라이언트로 봅니다. 기본은 아무것도 믿지 않아 peer 주소가 클라이언트이므로, load balancer 뒤에 배포할 때는 반드시 설정해야 합니다(그렇지 않으면 모든 사용자가 balancer 주소의 rate limit을 나눠 씁니다). rate limit 버킷은 인스턴스별로 따로 둡니다. 여러 주소에서 나눠 시도하는 추측은 IP 단위 제한으로 막을 수 없으므로, 비밀값을 비교하는 곳(비밀번호 로그인, 입력한 OTP, MFA factor 코드, 재인증 nonce)은 계정별 실패도 셉니다(`dilion_auth.auth_attempts`, 계정 식별자는 해시로만 저장). 15분 안에 한도(비밀번호 10회, 나머지 5회)를 넘으면 창이 지날 때까지 올바른 값도 429로 거부하고, 성공하면 초기화합니다. 인스턴스 관리자가 정하는 URL로 나가는 호출(인증 훅, 커스텀·SSO provider와 그 discovery가 가리키는 endpoint, privacy webhook destination)은 공인 주소에만 연결하며, `DILION_OUTBOUND_ALLOWED_NETWORKS`(`WithOutboundNetworks`)에 같은 형식으로 적은 사설망은 예외입니다. 코드로 정의한 provider(`WithProviderSource`)는 운영자 자신의 것이라 제한하지 않습니다. webhook destination에는 32바이트 이상의 secret이 필수입니다.

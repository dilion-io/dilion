...
- **신뢰할 proxy와 외부 호출 허용 망:** `X-Forwarded-For`는 클라이언트가 마음대로 보낼 수 있으므로, `DILION_TRUSTED_PROXIES`(`WithTrustedProxies`)에 적은 proxy가 전달한 경우에만 믿습니다. 값은 쉼표로 구분한 IP·CIDR이고 `*`는 모두 신뢰(기존 동작)입니다. 오른쪽부터 신뢰하는 proxy를 건너뛰고 처음 만나는 신뢰하지 않는 주소를 클라이언트로 봅니다. 기본은 아무것도 믿지 않아 peer 주소가 클라이언트이므로, load balancer 뒤에 배포할 때는 반드시 설정해야 합니다(그렇지 않으면 모든 사용자가 balancer 주소의 rate limit을 나눠 씁니다). rate limit 버킷은 인스턴스별로 따로 둡니다. 인스턴스 관리자가 정하는 URL로 나가는 호출(인증 훅, 커스텀·SSO provider와 그 discovery가 가리키는 endpoint, privacy webhook destination)은 공인 주소에만 연결하며, `DILION_OUTBOUND_ALLOWED_NETWORKS`(`WithOutboundNetworks`)에 같은 형식으로 적은 사설망은 예외입니다. 코드로 정의한 provider(`WithProviderSource`)는 운영자 자신의 것이라 제한하지 않습니다. webhook destination에는 32바이트 이상의 secret이 필수입니다.

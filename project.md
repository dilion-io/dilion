...
- **멀티 DB 인스턴스 (커스터마이징 전용):** 하나의 서버가 완전히 격리된 여러 DB(인스턴스마다 자체 `auth.*`+`dilion_*`)를 서비스할 수 있습니다. `InstanceResolver` 인터페이스가 인스턴스별 리소스(DB pool, KMS, 컴플라이언스 정책, PII field 정의)를 **동적으로** 제공하며, 요청의 인스턴스 선택은 embedder가 context 주입으로 수행합니다. **내장 HTTP 라우팅이나 토큰-인스턴스 결합은 제공하지 않습니다** — 자격증명을 인스턴스에 결합하는 책임은 embedder에게 있습니다. 기본 데몬과 예제는 단일 인스턴스만 사용합니다.

...
- **멀티 DB 인스턴스 (커스터마이징 전용):** 하나의 서버가 완전히 격리된 여러 DB(테넌트보다 큰 범위 — 인스턴스마다 자체 `auth.*`+`dilion_*`)를 서비스할 수 있습니다. `InstanceResolver` 인터페이스가 인스턴스별 리소스(DB pool, KMS, 컴플라이언스 정책, PII field 정의)를 **동적으로** 제공하며, 요청의 인스턴스 선택은 embedder가 context 주입으로 수행합니다. **내장 HTTP 라우팅이나 토큰-인스턴스 결합은 제공하지 않습니다** — 자격증명을 인스턴스에 결합하는 책임은 embedder에게 있습니다. 기본 데몬과 예제는 단일 인스턴스만 사용합니다.
...
- 계정 생성 시 `dilion_privacy.subject_policies`에 컴플라이언스 코드가 **명시적으로 할당**됩니다(배포자/테넌트의 명시 지정, 없으면 `default-policy`). 할당 근거(`source`)를 함께 기록합니다.
...
- 모든 권한은 **(project_id, permission) 쌍**으로 테넌트에 스코핑되며 cross-project 권한은 없습니다. **deny-by-default** — `pii.read` 없는 모든 조회는 masked 프로젝션(§2.6)입니다.
- 내장 permission:
...
dilion_privacy.destinations (id, project_id, type,    -- webhook | connector
...
-- 권한 (관리 평면 RBAC — §2.11)
dilion_authz.permissions      (project_id, name)      -- 사용자 정의 permission 등록 (네임스페이스 필수)
dilion_authz.roles            (id, project_id, name, permissions[])   -- 내장 + 사용자 정의
dilion_authz.role_assignments (project_id, actor_id, role_id,
                               granted_by, granted_at,
                               revoked_by, revoked_at)  -- 이력 보존 (고시 §5 기록 의무, CC6.3 리포트)
dilion_authz.api_keys         (id, project_id, key_hash, scopes[],
                               expires_at, last_used_at)

-- 감사
dilion_audit.events   (event_id, project_id, actor_id, actor_type, action, resource,
...
9. **Authorization** — 관리 평면 내장 RBAC(테넌트 스코프, deny-by-default), 사용자 정의 permission·role, 이력 보존형 할당 + recertification 리포트, scoped API key, 관리 평면 MFA 기본 요구

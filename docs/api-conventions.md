...
- `code`: 기계 판독용 snake_case 문자열 (frontend 분기 기준). 목록: `validation_failed`, `unauthenticated`, `permission_denied`, `not_found`, `conflict`, `idempotency_conflict`, `legal_hold_active`, `policy_violation`, `rate_limited`, `reauthentication_needed`, `insufficient_aal`, `internal`.

...
create index if not exists role_assignments_actor_idx
    on dilion_authz.role_assignments (actor_id)
    where revoked_at is null;

create index if not exists role_assignments_role_idx
    on dilion_authz.role_assignments (role_id);
...
create index if not exists events_actor_idx   on dilion_audit.events (actor_id, created_at desc);
...
insert into dilion_authz.permissions (name, builtin) values
    ...
    ('pii.reveal',              true),
    ('pii.export',              true),
    ('privacy.requests.manage', true),
    ('holds.manage',            true),
    ('policies.manage',         true),
    ('destinations.manage',     true),
    ('keys.manage',             true),
    ('audit.read',              true)
...
insert into dilion_authz.roles (id, name, permissions, builtin) values
    ('role_00000000000000000000000000000001', 'viewer',
        array['users.read'], true),
    ('role_00000000000000000000000000000002', 'support',
        array['users.read','pii.read'], true),
    ('role_00000000000000000000000000000003', 'privacy-officer',
        array['users.read','pii.read','pii.export','privacy.requests.manage','holds.manage'], true),
    ('role_00000000000000000000000000000004', 'security-admin',
        array['users.read','pii.read','pii.export','privacy.requests.manage','holds.manage','audit.read','keys.manage'], true),
    ('role_00000000000000000000000000000005', 'owner',

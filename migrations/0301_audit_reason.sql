...
-- 0300 already creates events_action_idx as (action, created_at desc), which
-- serves every action filter this API issues; the statement below is therefore
-- a no-op there. It is kept so the file also applies to installations whose
-- 0300 predates that index.
...
-- desc).
create index if not exists events_created_keyset_idx
    on dilion_audit.events (created_at desc, event_id desc);

...
create table if not exists dilion_privacy.consent_state (
    ...
    primary key (project_id, user_id, purpose)
);
...
with latest as (
    select distinct on (project_id, user_id, purpose)
        project_id, user_id, purpose, action,
        coalesce(policy_version, '') as policy_version, created_at
    from dilion_privacy.consent_events
    where action in ('GRANT', 'WITHDRAW')
    order by project_id, user_id, purpose, created_at desc, id desc
), notices as (
    select project_id, user_id, purpose, max(created_at) as noticed_at
    from dilion_privacy.consent_events
    where action = 'RECONFIRM_NOTICE'
    group by project_id, user_id, purpose
)
insert into dilion_privacy.consent_state
    (project_id, user_id, purpose, granted, policy_version, updated_at, last_notice_at)
select l.project_id, l.user_id, l.purpose, l.action = 'GRANT',
       l.policy_version, l.created_at, n.noticed_at
from latest l
left join notices n using (project_id, user_id, purpose)
on conflict (project_id, user_id, purpose) do nothing;

create index if not exists consent_state_reconfirm_due_idx
    on dilion_privacy.consent_state (project_id, next_reconfirm_at, user_id, purpose)
